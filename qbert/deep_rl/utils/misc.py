...
def run_episodes(agent):
    ...
    lastPrint = 0
    while True:
        if agent.total_steps - lastTest > config.test_interval:
            lastTest = agent.total_steps
            totalR = 0
            for _i in range(config.test_repetitions):
                reward, step = agent.episode(deterministic = True)
                totalR += reward
            avg_test_rewards.append(totalR/float(config.test_repetitions))


        if agent.total_steps - lastPrint > config.print_test:
            lastPrint = agent.total_steps
