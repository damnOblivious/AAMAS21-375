...
PUDDLE_COLOR = formatColor(.5,.5,.5)
PUDDLE_SIZE = 0.6
...
class PacmanGraphics:
    ...
    def drawStaticObjects(self, state):
        ...
        self.puddle = self.drawPuddle(layout.puddles)
    ...
    def drawPuddle(self, puddleMatrix ):
        puddleImages = []
        color = PUDDLE_COLOR
        for xNum, x in enumerate(puddleMatrix):
            if self.capture and (xNum * 2) <= puddleMatrix.width: color = TEAM_COLORS[0]
            if self.capture and (xNum * 2) > puddleMatrix.width: color = TEAM_COLORS[1]
            imageRow = []
            puddleImages.append(imageRow)
            for yNum, cell in enumerate(x):
                if cell: # There's food here
                    screen = self.to_screen((xNum, yNum ))
                    dot = circle( screen,
                                  PUDDLE_SIZE * self.gridSize,
                                  outlineColor = color, fillColor = color,
                                  width = 1)
                    imageRow.append(dot)
                else:
